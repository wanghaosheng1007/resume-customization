...
name: tailor-resume
description: >-
  Tailor an English resume to a job description using sources/ materials.
  Use when the user provides a JD, asks to customize a CV, rewrite experience
  bullets, or runs /tailor-resume. Produces resume.json, resume.md, and evidence.md.
...
- If user references `@job-descriptions/foo.md`, read that file and derive slug from filename when sensible.
...
One section per bullet (and per project bullet if any). Follow @.cursor/skills/tailor-resume/templates/evidence.md.template
...
Render human-readable resume from JSON. Follow @.cursor/skills/tailor-resume/templates/resume.md.template
...
[paste JD or @job-descriptions/demo-acme-senior-backend.md]
```
...
- Fabricate content (see project rule `resume-integrity`)
